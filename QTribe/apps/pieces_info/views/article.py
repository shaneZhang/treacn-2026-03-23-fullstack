...
class MyArticle(LoginRequiredMixin, View):
    def get(self, request):
        ...
        return render(request, 'pieces/my_article.html', context)
...
class StarArticleList(LoginRequiredMixin, View):
    def get(self, request):
        ...
        # 使用分页工具
        context = paginate_queryset(request, articles, per_page=2)
        context['collection_ids'] = collection_ids
        return render(request, 'pieces/star_article.html', context)
...
class CollectArticleList(LoginRequiredMixin, View):
    def get(self, request):
        ...
        return render(request, 'pieces/collect_article.html', context)


class ArticleSearchView(PaginationMixin, SearchView):
