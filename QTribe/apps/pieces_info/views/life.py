...
class LifeSearchView(View):
    """生活搜索视图 - 使用 Django ORM 替代 Elasticsearch"""
    ...
    def get(self, request):
        ...
        # 使用 Django ORM 进行搜索
        if query:
            results = LifeModel.objects.filter(
                Q(copy__icontains=query)
            ).order_by('-is_top', '-id')
        else:
            results = LifeModel.objects.none()
        ...
        star_ids = []  # 接受用户已点过赞的生活
        collection_ids = []  # 接受用户已收藏的生活

        if request.user.is_authenticated:
            for life in piece_list:
